...
-- ---------------------------------------------------------------------------
-- Netrw: ディレクトリ移動時に cwd を自動追従させる
-- ---------------------------------------------------------------------------
-- なぜこの設定が必要か：
--   デフォルト（netrw_keepdir = 1）では `:Ex` でディレクトリを移動しても
--   Neovim の cwd（カレントワーキングディレクトリ）は変わらない。
--   その結果、lazygit が「cwd 基準で git リポジトリを探す」ため、
--   移動先の別リポジトリを認識できず正しく動作しない。
--   0 に設定することで Netrw の移動に cwd が追従し、lazygit が
--   正しいリポジトリを見つけられるようになる。
